...
import com.github.shareme.gwsmaterialmatters.core.advancerv.swipeable.BaseSwipeableItemAdapter;
...
import com.github.shareme.gwsmaterialmatters.core.advancerv.swipeable.SwipeResultActionDefault;
import com.github.shareme.gwsmaterialmatters.core.advancerv.swipeable.SwipeableItemAdapter;
import com.github.shareme.gwsmaterialmatters.core.advancerv.swipeable.SwipeableItemInternalUtils;
import com.github.shareme.gwsmaterialmatters.core.advancerv.utils.BaseWrapperAdapter;
import com.github.shareme.gwsmaterialmatters.core.advancerv.utils.WrapperAdapterUtils;
...
class DraggableItemWrapperAdapter<VH extends RecyclerView.ViewHolder> extends BaseWrapperAdapter<VH> implements SwipeableItemAdapter<VH> {
  ...
  public void onBindViewHolder(VH holder, int position, List<Object> payloads) {
    if (isDragging()) {
      ...
      if (itemId == draggingItemId && holder != mDraggingItemViewHolder) {
        ...
        if (LOCAL_LOGI) {
          Timber.i(TAG, "a new view holder object for the currently dragging item is assigned");
        }
  ...
  // NOTE: This method is called from RecyclerViewDragDropManager
    ...
    if (LOCAL_LOGD) {
      Timber.d(TAG, "onDragItemStarted(holder = " + holder + ")");
    }
  ...
  // NOTE: This method is called from RecyclerViewDragDropManager
    ...
    if (LOCAL_LOGD) {
      Timber.d(TAG, "onDragItemFinished(result = " + result + ")");
    }
  ...
  private void onDraggingItemRecycled() {
    if (LOCAL_LOGI) {
      Timber.i(TAG, "a view holder object which is bound to currently dragging item is recycled");
    }
    mDraggingItemViewHolder = null;
  ...
  boolean canStartDrag(RecyclerView.ViewHolder holder, int position, int x, int y) {
    if (LOCAL_LOGV) {
      Timber.v(TAG, "canStartDrag(holder = " + holder + ", position = " + position + ", x = " + x + ", y = " + y + ")");
    }
  ...
  boolean canDropItems(int draggingPosition, int dropPosition) {
    if (LOCAL_LOGV) {
      Timber.v(TAG, "canDropItems(draggingPosition = " + draggingPosition + ", dropPosition = " + dropPosition + ")");
    }
  ...
  ItemDraggableRange getItemDraggableRange(RecyclerView.ViewHolder holder, int position) {
    if (LOCAL_LOGV) {
      Timber.v(TAG, "getItemDraggableRange(holder = " + holder + ", position = " + position + ")");
    }
  ...
  // NOTE: This method is called from RecyclerViewDragDropManager
    ...
    if (LOCAL_LOGD) {
      Timber.d(TAG, "onMoveItem(fromPosition = " + fromPosition + ", toPosition = " + toPosition + ")");
    }
