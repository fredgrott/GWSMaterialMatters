 ...
 *
 */
package com.github.shareme.gwsmaterialmatters.core.vanimator;
...
public class SvgPathParser {
  ...
  public static Path tryParsePath(String dAttributeOfPath) {
    try {
      ...
      Timber.e("parse svg path error", e);
    }
    return null;
  }
    ...
    while (helper.getPosition() < n) {
      ...
      switch (cmd) {
        ...
        case 'm': {
          ...
          Timber.d(String.format("move to: [%s,%s]", x, y));
          if (cmd == 'm') {
            subPathStartX += x;
            subPathStartY += y;
            path.rMoveTo(x, y);
        ...
        case 'Z':
        case 'z': {
          Timber.d(String.format("close, move to: [%s,%s]", subPathStartX, subPathStartY));
          path.close();
        ...
        case 'l': {
          float x = helper.nextFloat();
          float y = helper.nextFloat();
          Timber.d(String.format("line to: [%s,%s]", x, y));
          if (cmd == 'l') {
            path.rLineTo(x, y);
            lastX += x;
            lastY += y;
        ...
        case 'h': {
          float x = helper.nextFloat();
          Timber.d(String.format("horizontal line to: [%s]", x));
          if (cmd == 'h') {
            path.rLineTo(x, 0);
            lastX += x;
          } else {
        ...
        case 'v': {
          float y = helper.nextFloat();
          Timber.d(String.format("vertical line to: [%s]", y));
          if (cmd == 'v') {
            path.rLineTo(0, y);
            lastY += y;
          } else {
        ...
        case 'c': {
          ...
          float x = helper.nextFloat();
          float y = helper.nextFloat();
          Timber.d(String.format("cubic to: [%s,%s][%s,%s][%s,%s]", x1, y1, x2, y2, x, y));
          if (cmd == 'c') {
            x1 += lastX;
            x2 += lastX;
            x += lastX;
        ...
        case 's': {
          ...
          float x = helper.nextFloat();
          float y = helper.nextFloat();
          Timber.d(String.format("cubic to: [%s,%s][%s,%s]", x2, y2, x, y));
          if (cmd == 's') {
            x2 += lastX;
            x += lastX;
            y2 += lastY;
        ...
        case 'a': {
          ...
          float x = helper.nextFloat();
          float y = helper.nextFloat();
          Timber.d(String.format("arc to: [%s,%s][%s][%s,%s][%s,%s]", rx, ry, theta, largeArc, sweepArc, x, y));
